...
// worker calls (all against the same TOKENS binding), so they run concurrently
// rather than stacking their latency. Usage previously round-tripped from the
// client after hydration (GET /api/usage in onMount), which meant a visible
// loading flash on every dashboard visit; fetchUsage() never throws (it
// degrades to free-tier defaults on failure, same as /api/usage always has),
// so folding it into this Promise.all can't turn a key/bucket success into a
// 500.
