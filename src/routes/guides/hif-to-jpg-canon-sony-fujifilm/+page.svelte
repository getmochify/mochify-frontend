<script>
    ...
    const faqs = [
        ...
        { q: 'Can I convert HIF to JPG without losing quality?', a: `There is always some information loss going from 10-bit HIF to 8-bit JPEG; the bit depth reduction is inherent. In practice, the difference is hard to see in most shots. Using a quality-aware encoder like jpegli and setting JPEG quality at 85 or above minimises visible banding and artefacts. The most noticeable case is Canon HDR PQ files, where the HDR-to-SDR tone-mapping step alters the look more significantly than bit depth alone.` },
        ...
        { q: 'What JPEG quality setting should I use when converting from HIF?', a: `85–90 is the practical sweet spot for most photography: visually clean for display and printing, with file sizes that work for sharing and web delivery. Above 95 adds bytes with minimal visible gain. Below 80 risks visible artefacts, particularly in 10-bit-to-8-bit conversions. Mochify's defaults are calibrated for high quality output with jpegli's adaptive quantisation handling the subtle detail.` },
    ];
...
<svelte:head>
    ...
    <script type="application/ld+json">
        ...
        "mainEntity": [
            ...
            { "@type": "Question", "name": "Can I convert HIF to JPG without losing quality?", "acceptedAnswer": { "@type": "Answer", "text": "There is always some information loss going from 10-bit HIF to 8-bit JPEG; the bit depth reduction is inherent. In practice, the difference is hard to see in most shots. Using a quality-aware encoder like jpegli and setting JPEG quality at 85 or above minimises visible banding and artefacts. The most noticeable case is Canon HDR PQ files, where the HDR-to-SDR tone-mapping step alters the look more significantly than bit depth alone." } },
            ...
            { "@type": "Question", "name": "What JPEG quality setting should I use when converting from HIF?", "acceptedAnswer": { "@type": "Answer", "text": "85-90 is the practical sweet spot for most photography. Above 95 adds bytes with minimal visible gain. Below 80 risks visible artefacts, particularly in 10-bit-to-8-bit conversions. Mochify's defaults are calibrated for high quality output with jpegli's adaptive quantisation." } }
        ]
        }
    </script>
</svelte:head>
...
<article class="bg-white rounded-none md:rounded-3xl pt-6 px-6 pb-8 md:p-12 border-x md:border border-pink-50 shadow-sm relative overflow-hidden">
    ...
    <header class="mb-12 border-b border-pink-50 pb-12">
        ...
        <p class="text-xl text-[#6C3F31] opacity-90 leading-relaxed max-w-2xl mb-8">
            HIF files come off professional mirrorless cameras full of 10-bit colour and punishing dynamic range, and then promptly refuse to open on half the computers in the world. If you need to convert HIF to JPG and want to understand what you are trading away in the process, this guide covers every method, on every platform, for every camera brand.
        </p>
    ...
    <div class="space-y-8 text-lg text-[#6C3F31] leading-relaxed">
        ...
        <section id="what-is-a-hif-file">
            ...
            <p>A HIF file is a still image from a professional mirrorless camera saved in the HEIF (High Efficiency Image File Format) container, with <code class={inlineCode}>.HIF</code> as the vendor-specific file extension. Fujifilm's official camera manual states this directly: "HEIF pictures are stored on the memory card as files with the extension <code class={inlineCode}>.HIF</code>." Canon's Digital Photo Professional software uses the same <code class={inlineCode}>.HIF</code> convention for its HEIF save-as type. Under the hood, HIF uses HEVC (H.265) compression inside an ISO Base Media File Format container (the same family that underpins MP4 video) to pack more colour depth into a smaller file than traditional JPEG.</p>
            ...
            <p><strong>Sony</strong> Alpha bodies (ILCE-7M4/A7 IV, ILCE-6700, and other recent Alpha bodies) offer a JPEG/HEIF toggle in the Image Quality menu. Photographers can choose HEIF(4:2:0) or HEIF(4:2:2), the latter keeping more colour detail. Sony's documentation describes this as "recording images with a rich 10-bit colour depth gradation expression," with optional HLG (Hybrid Log-Gamma) stills for a second flavour of wide dynamic range.</p>
            ...
            <p>The core case for HIF is 10-bit colour and roughly half the file size at comparable quality. Sony's HEIF documentation puts the compression efficiency at approximately twice that of JPEG at equal perceived quality. One Sony user comparison found 8-bit JPEGs from the same scene at around 14.3 MB against 10-bit HEIF(4:2:2) at about 5.7 MB. The extra bit depth also reduces banding in smooth gradients (skies, skin tones, subtle colour transitions) that shows up as posterisation in 8-bit JPEG when you push editing further in post.</p>
        ...
        <section id="why-hif-files-wont-open">
            ...
            <p>macOS uses an OS-level codec layer, so Photos and Preview can render <code class={inlineCode}>.HIF</code> files even when third-party web services cannot. The web service decodes the file using its own stack, and that stack may lack an HEVC licence or HEIF parser. The same file that opens fine in Finder can be rejected when you try to upload it elsewhere; the file is not corrupt, the target just cannot read it.</p>
        </section>
        ...
        <section id="what-you-lose-and-keep">
            ...
            <p>The core trade-off is bit depth. HIF files from professional cameras are typically 10-bit per channel. Standard JPEG is 8-bit. That means moving from 1,024 tonal steps per channel to 256, and in smooth gradients like skies and skin, that compression can show up as banding or posterisation.</p>
            ...
            <p>Fujifilm's film simulations are baked into both JPEG and HIF renders at capture. When you convert a HIF to JPG with a colour-profile-aware converter, the film sim look survives. The risk is converters that ignore embedded ICC profiles or apply heavy re-compression on top, which can subtly shift Velvia saturation or Classic Chrome's roll-off. A quality-aware encoder that respects the colour profile will preserve the look as intended.</p>
        </section>
