...
<article class="bg-white rounded-none md:rounded-3xl pt-6 px-6 pb-8 md:p-12 border-x md:border border-pink-50 shadow-sm relative overflow-hidden">
    ...
    <div class="space-y-8 text-lg text-[#6C3F31] leading-relaxed">
        ...
        <section id="the-problem" class="scroll-mt-24">
            ...
            <p>From our side the compression was working: sub-second responses, large reductions, the right output format. The bytes were getting back to the client. The handoff from the chat client to the user was where things came apart, and for a tool that positions itself as the image toolkit for AI-native workflows, "the compression works but you can't actually get your file" is not an acceptable outcome. The full breakdown lives in <a href="/guides/how-the-mochify-mcp-server-works" class="text-[#F06292] hover:text-[#D81B60] underline decoration-2 underline-offset-2 transition-colors">how the Mochify MCP server works</a>.</p>
        </section>
        ...
        <section id="your-workflow" class="scroll-mt-24">
            ...
            <p class="mb-4">If you need a stronger end-to-end privacy posture, install the local binary with <code class="bg-pink-50 text-[#D81B60] px-2 py-0.5 rounded font-mono text-sm">brew install mochify</code> and use either <code class="bg-pink-50 text-[#D81B60] px-2 py-0.5 rounded font-mono text-sm">mochify serve</code> (chat-driven) or the CLI direct (build pipelines, batch jobs, anything scripted). Compressed bytes never sit on Mochify infrastructure beyond the encoding step itself. If you need to take Mochify off the path entirely, self-host the engine via the <a href="/guides/self-hosting-image-optimization-docker" class="text-[#F06292] hover:text-[#D81B60] underline decoration-2 underline-offset-2 transition-colors">Docker self-hosting guide</a>.</p>
