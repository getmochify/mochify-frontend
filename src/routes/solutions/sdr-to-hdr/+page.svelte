<script lang="ts">
    ...
    // synthesises a plausible one when it does not. Core reports them separately
    ...
    const lanes = [
        {
            ...
            sources: 'Ultra HDR JPEGs from Pixel and recent Android phones, Apple Adaptive HDR JPEGs shared out of iPhone',
        ...
        },
        {
            ...
            detail: 'A gain map is synthesised from the highlights already in the frame: a smooth ramp that starts around three-quarters brightness and climbs to roughly 1.5 stops, tempered by local contrast so a bright sky lifts differently from a white shirt.',
            tone: 'amber'
        }
    ];
    ...
    const formatSupport = [
        ...
        { fmt: 'JXL', verdict: 'partial', note: 'No gain map concept. HDR lives in the pixels as linear float, so it works from a real HDR source but there is nothing to synthesise into.' },
    ...
    const disqualifiers = [
        ...
        { op: 'Background removal', why: 'Matting runs on standard-range pixels and composites against 0-255 colours, which are meaningless against the 0-1 scale HDR works in.' },
...
<svelte:head>
    <title>SDR to HDR Converter - Add a Gain Map to Any Photo | Mochify</title>
    <meta name="description" content="Turn standard photos into Ultra HDR JPEGs. Mochify preserves a camera's own gain map when your file has one and synthesises a plausible one when it does not.">
    <meta property="og:title" content="SDR to HDR Converter - Mochify">
    <meta property="og:description" content="Add an HDR gain map to any photo. Preserved when your camera captured it, generated when it did not. Still opens everywhere.">
    ...
    <script type="application/ld+json">
        {
            ...
            "name": "Mochify SDR to HDR Converter",
            "operatingSystem": "Any",
            "applicationCategory": "MultimediaApplication",
            "applicationSubCategory": "Image Converter",
            "url": "https://mochify.app/solutions/sdr-to-hdr",
            "description": "Convert standard dynamic range photos into Ultra HDR JPEGs, preserving an existing gain map or synthesising one.",
            "offers": {
                "@type": "Offer",
                "price": "0",
                "priceCurrency": "USD",
            ...
            "featureList": [
                ...
                "Synthesise a gain map for plain SDR photos",
...
<div class="relative max-w-5xl mx-auto px-4 pt-7 pb-12 sm:px-6 lg:px-8 w-full flex-grow">
    ...
    <div class="text-center mb-12 space-y-6">
        ...
        <p class="text-lg text-[#6C3F31] font-medium max-w-2xl mx-auto leading-relaxed">
            Give any photo a <strong class="text-[#4A2C2C]">gain map</strong> so highlights can go brighter than white on an HDR display. If your camera already captured one, we carry it through untouched. If it did not, we build a plausible one.
        </p>
    </div>
    ...
    <section class="mt-10 max-w-4xl mx-auto">
        ...
        <div class="mt-4 bg-[#FFF7E6] border border-[#FDE68A] rounded-2xl px-6 py-5">
            <p class="text-sm text-[#7C4A03] leading-relaxed">
                ...
                Nothing can retrieve detail your camera never wrote down. Synthesis reads the highlights that did survive and builds a brightness curve from them, which looks convincing on an HDR screen but is an effect rather than measured data. Every response says which one you got: the <span class="font-mono font-bold">X-Mochify-HDR</span> header returns <span class="font-mono font-bold">true</span> for preserved, <span class="font-mono font-bold">generated</span> for synthesised, and <span class="font-mono font-bold">false</span> when the result carries none.
            </p>
        </div>
    </section>
        ...
        <div class="grid md:grid-cols-2 gap-12 items-start">
            <div class="space-y-8">
                ...
                <div class="bg-white p-7 rounded-2xl border border-pink-50 shadow-sm space-y-3">
                    <h3 class="font-bold text-[#4A2C2C] text-sm uppercase tracking-widest opacity-70">How the synthesised map is built</h3>
    ...
    <section class="mt-20 max-w-4xl mx-auto">
        ...
        <p class="text-[#6C3F31] leading-relaxed mb-6 max-w-2xl">
            One parameter. Use <span class="font-mono font-bold text-[#4A2C2C]">hdr=true</span> to preserve only, or <span class="font-mono font-bold text-[#4A2C2C]">hdr=generate</span> to preserve and synthesise, which is what this page sends.
        </p>
