<script lang="ts">
	...
	async function submit() {
		...
		try {
			...
			// fully) filled by the time it flips to 'uploading'.
			if (
				uploadMode === 'image' &&
				...
				{#each files as file, i}
					<div class="group animate-fade-in relative flex-shrink-0">
						<div class="liquid-bubble h-16 w-16 overflow-hidden rounded-2xl p-1">
							...
							{:else}
								<div
									class="flex h-full w-full flex-col items-center justify-center gap-0.5 rounded-xl bg-[#FFF0F3]"
								>
									<svg
									...
									<span class="text-[8px] font-bold tracking-wide text-[#F06292]/70 uppercase">
										{file.name.split('.').pop()}
									</span>
								</div>
							{/if}
